setwd("C:/Users/94876/Desktop/全国蓝藻数据/中性模型")
##### Import Data #####
...
spp<-read.csv('硅藻北方.csv',head=T,stringsAsFactors=F,row.names=1,sep = ",")
spp<-t(spp)
...
write.csv(p, file = "硅藻北方p.csv")
write.csv(freq, file = "硅藻北方freq.csv")
write.csv(freq.pred, file = "硅藻北方freq.pred.csv")
...
inter.col[bacnlsALL$freq <= bacnlsALL$Lower]<-'#F89C0D'#出现频率低于中性群落模型预测的部分
inter.col[bacnlsALL$freq >= bacnlsALL$Upper]<-'#35BABF'#出现频率高于中性群落模型预测的部分
